...
from triton._internal_testing import is_cuda, is_hip, is_hip_cdna2, is_hip_cdna3, is_hip_cdna4, is_hip_gfx11, is_hip_gfx12
    ...
    if is_cuda():
        ...
        if  src_dtype == 'float8e4b15':
            ...
            with pytest.raises(triton.CompilationError, match="not supported in this architecture"):
                launch_exhaustive_populate(getattr(tl, src_dtype), 0, 65536, False, 8, 0x7f, device=device)
            return
        if src_dtype in ('float8e4b8', 'float8e5b16') and (is_hip_cdna2() or is_hip_gfx11() or is_hip_gfx12()):
...
def test_typeconvert_downcast(src_dtype, dst_dtype, rounding, max_repr, device):
    ...
    if is_hip():
        if dst_dtype in ('float8e4b8', 'float8e5b16') and (is_hip_cdna2() or is_hip_gfx11() or is_hip_gfx12()):
