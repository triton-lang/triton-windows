...
from triton.profiler.profile import _check_env, _select_backend


def _select_backend_with_env() -> str:
...
def test_context(source: str, tmp_path: pathlib.Path):
    ...
    session_id = libproton.start(str(temp_file.with_suffix("")), source, "tree", _select_backend_with_env())
...
def test_session(tmp_path: pathlib.Path):
    ...
    session_id = libproton.start(str(temp_file.with_suffix("")), "shadow", "tree", _select_backend_with_env())
...
def test_add_metrics(tmp_path: pathlib.Path):
    ...
    libproton.start(str(temp_file.with_suffix("")), "shadow", "tree", _select_backend_with_env())
    id1 = libproton.record_scope()
    libproton.enter_scope(id1, "one")
